...
export default function LanguageSwitcher() {
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger className="flex items-center gap-1 rounded-md hover:bg-secondary hover:text-secondary-foreground transition-colors py-2 px-3">
        <Globe className="h-4 w-4 mr-1" />
        <span className="hidden md:inline">{languageNames[language]}</span>
        <span className="md:hidden">{flags[language]}</span>
